...
def _transpose_pads_to_padtype(kernel_sdims, strides, padding):
  """Finds the padding type for a transpose convolution."""
  # This is simply checking agreement with lax._conv_transpose_padding.
  is_valid = True
  is_same = True
  for k, s, (begin, end) in zip(kernel_sdims, strides[1:-1], padding):
...
def _normalize_dilations(lhs_dilation, rhs_dilation, is_conv1d):
  if is_conv1d:
    lhs_dilation = list(lhs_dilation) + [1]
    rhs_dilation = list(rhs_dilation) + [1]
  return lhs_dilation, rhs_dilation

def _normalize_window_strides(window_strides, is_conv1d):
  ...
  if is_conv1d:
    # For a conv1d, the window_strides will be either an int, a length 1 list,
    # or a length 3 list. Insert stride 1 for the dummy dimension, then use the
    # usual 2d logic.
    if type(window_strides) == int:
      window_strides = [window_strides]
    if len(window_strides) == 1:
      window_strides = list(window_strides) + [1]
    elif len(window_strides) == 3:
      # Increase to length 4.
      window_strides = list(window_strides)[:-1] + [1] + [window_strides[-1]]
...
def _conv_general_dilated(
  ...
  tf_window_strides = _normalize_window_strides(window_strides, is_conv1d)
  lhs_dilation, rhs_dilation = _normalize_dilations(
      lhs_dilation, rhs_dilation, is_conv1d)
  ...
  *rhs_spatial_shapes, _, rhs_out_channel = rhs.shape
  ...
  if is_transpose:
    padding_type = _transpose_pads_to_padtype(
        rhs_spatial_shapes, tf_window_strides, padding)
  else:
